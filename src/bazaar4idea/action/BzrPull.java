...
import bazaar4idea.command.BzrTask;
import bazaar4idea.command.BzrTaskResultHandlerAdapter;
...
public class BzrPull extends BzrRepositoryAction {
  ...
  protected void perform(@NotNull final Project project,
                         @NotNull final List<VirtualFile> bzrRoots,
                         @NotNull final VirtualFile defaultRoot,
                         final Set<VirtualFile> affectedRoots,
                         final List<VcsException> exceptions) throws VcsException {
    final BzrPullDialog dialog = new BzrPullDialog(project, bzrRoots, defaultRoot);
    dialog.show();
    if (!dialog.isOK()) {
      return;
    }
...
//        pullTask.setProgressAnalyzer(new BzrStandardProgressAnalyzer());
        pullTask.execute(true, false, new BzrTaskResultHandlerAdapter() {
          ...
          protected void onSuccess() {
            ...
            BzrMergeUtil.showUpdates(BzrPull.this, project, exceptions, root, currentRev, beforeLabel, getActionName(),
                    ActionInfo.UPDATE);
