 ...
 *
 * <b>Cancellation</b> is implemented with a {@link java.util.Timer} which checks whether the ProgressIndicator was
 * cancelled and kills the BzrHandler in that case.
 *
 * A BzrTask may be executed synchronously ({@link #executeModal()} or asynchronously
 * ({@link #executeAsync(BzrTask.ResultHandler)}.
 * Result of the execution is encapsulated in {@link BzrTaskResult}.
 *
 * <p>
 * <b>BzrTaskResultHandler</b> is called from AWT-thread.
 * Use {@link #setExecuteResultInAwt(boolean) setExecuteResultInAwt(false)} to execute the result
 * {@link com.intellij.openapi.application.Application#executeOnPooledThread(Runnable) on the pooled thread}.
 * </p>
 *
...
//  private BzrProgressAnalyzer myProgressAnalyzer;
  ...
  private void commonOnSuccess(final Object LOCK, final BzrTaskResultHandler resultHandler) {
    BzrTaskResult res = !myHandler.errors().isEmpty() ? BzrTaskResult.BZR_ERROR : BzrTaskResult.OK;
...
//  }
  ...
  /**
   * We're using this interface here to work with Task, because standard {@link com.intellij.openapi.progress.Task#run(com.intellij.openapi.progress.ProgressIndicator)}
   * is busy with timers.
   */
